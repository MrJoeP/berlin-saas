...
export async function handle(job: Job, client: SupabaseClient): Promise<Record<string, unknown>> {
  ...
  if (clusters.length === 0) return { message: "Keine validen Cluster, kein Digest erzeugt." };
  ...
  const analyses = await Promise.all(
    clusters.map(async (cluster) => {
      ...
      try {
        ...
        console.error(`deepSynthesize failed for cluster "${cluster.cluster_name}":`, err);
        return {
          cluster_name: cluster.cluster_name,
...
async function loadTrendMemory(client: SupabaseClient, companyId: string): Promise<TrendMemoryEntry[]> {
...
async function loadVoteSignals(client: SupabaseClient): Promise<ClusterVoteSignal[]> {
  const { data } = await client
    .from("votes")
  ...
  return [...scores.entries()].map(([target_id, score]) => ({ target_id, score }));
}
...
): Promise<Omit<ClusterAnalysis, "cluster_name" | "confidence" | "confidence_reason" | "trend_streak">> {
