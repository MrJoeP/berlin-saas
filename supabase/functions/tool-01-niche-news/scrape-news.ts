...
import { filterItems, type CompanySourceWithSource } from "../_shared/news_filter.ts";
...
export async function handle(job: Job, client: SupabaseClient): Promise<Record<string, unknown>> {
  ...
  // Pre-Filter Pipeline.
  const filtered = filterItems(rawItems, companySources as unknown as CompanySourceWithSource[], keywords)
